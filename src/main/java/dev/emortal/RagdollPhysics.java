    ...
    public RagdollPhysics(MinecraftPhysicsHandler physicsHandler, Player spawner, @Nullable PhysicsRigidBody torso, PlayerDisplayPart part, Instance instance, Vector3f position, Vector3f size, float mass) {
        ...
        if (torso != null) {
            ...
            Vector3f firstThing = switch (part) {
                ...
                case RIGHT_ARM -> new Vector3f((float)torsoSize.x() / 2f, (float)torsoSize.y() / 2f, 0f);
                case LEFT_ARM -> new Vector3f((float)-torsoSize.x() / 2f, (float)torsoSize.y() / 2f, 0f);
                case RIGHT_LEG -> new Vector3f(0.2f, (float)-torsoSize.y() / 2f, 0f);
                case LEFT_LEG -> new Vector3f(-0.2f, (float)-torsoSize.y() / 2f, 0f);
            ...
            Vector3f secondThing = switch (part) {
                ...
                case RIGHT_ARM -> new Vector3f((float)-limbSize.x() / 2f, (float)limbSize.y() / 2f, 0f);
                case LEFT_ARM -> new Vector3f((float)limbSize.x() / 2f, (float)limbSize.y() / 2f, 0f);
                case RIGHT_LEG -> new Vector3f(0f, (float)limbSize.y() / 2f, 0f);
                case LEFT_LEG -> new Vector3f(0f, (float)limbSize.y() / 2f, 0f);
                default -> throw new IllegalStateException("Unexpected value: " + part);
            };

            joint = new ConeJoint(torso, rigidBody, firstThing, secondThing);
